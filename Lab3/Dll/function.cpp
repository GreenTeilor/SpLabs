...
void replaceStringInMemory(Params* params)
{
	...
	if (process)
	{
		...
		{
			int sz = VirtualQueryEx(process, ptr, &memoryInfo, sizeof(MEMORY_BASIC_INFORMATION));
			if (sz == sizeof(MEMORY_BASIC_INFORMATION)) //memoryInfo shell contain MEMORY_BASIC_INFORMATION structure with info about the specified page range
			{
				if ((memoryInfo.State == MEM_COMMIT) && memoryInfo.AllocationProtect == PAGE_READWRITE)
				{
					...
					try {
						...
						if (ReadProcessMemory(process, ptr, buffer, memoryInfo.RegionSize, &bytesRead)) //Here read memory(ptr - pointer to the base address in the specified process from which to read, 
							//buffer - pointer to a buffer that receives the contents from the address space of the specified process, lpRead - number of bytes read)
						{
